...
function removeFromBag(itemId) {
  ...
  bagItems.forEach((bagItemId, index) => {
    console.log(`Comparing: itemId (${typeof itemId}) "${itemId}" with bagItemId (${typeof bagItemId}) "${bagItemId}"`);
  });

  // Filter out the item with the matching ID
  bagItems = bagItems.filter(bagItemId => {
    const result = bagItemId !== itemId;
    console.log(`Comparison result: ${bagItemId} !== ${itemId} => ${result}`);
    return result;
  });
...
function generateItemHTML(item) {
  console.log(`Generating HTML for item ID (${typeof item.id}): ${item.id}`);
